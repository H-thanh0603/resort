    ...
    <section className="bg-[#f7f5f0] py-32 lg:py-48">
      <div className="mx-auto max-w-[1440px] px-5 lg:px-12">
        <SceneHead
          ...
          title={<>Một bản đồ,<br /><span className="italic text-[#8c6d46]">tám thế giới.</span></>}
          lede="Tám địa điểm, một triết lý: mỗi nơi là một nhịp thở khác nhau của cùng một kỳ nghỉ."
          meta={["10.0245°N", "104.0322°E", "Bán đảo Nam"]}
        />
        ...
        <div className="mt-16 grid grid-cols-1 gap-12 lg:grid-cols-12">
              ...
              return (
                <li key={p.id} className="border-t hairline last:border-b">
                  <button
                    ...
                    onClick={() => pick(i)}
                    data-cursor="Xem"
                    className="group flex w-full items-baseline gap-5 py-5 text-left transition-all duration-500"
                    style={{ paddingLeft: on ? 12 : 0 }}
                  >
                    <span className={`font-mono text-xs transition-colors ${on ? "text-[#8c6d46]" : "text-black/35"}`}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span className="flex-1">
                      <span className={`font-display block text-3xl transition-all duration-500 sm:text-4xl ${on ? "text-[#161616]" : "text-black/40 group-hover:text-black/70"}`}>
                        {p.name}
                      </span>
                      <span
                      ...
                      >
                        <span className="overflow-hidden">
                          <span className="block pb-1 pt-2 text-sm font-light text-[#444748]">
                            <em className="text-[#8c6d46]">{p.line}.</em> {p.desc}
                          </span>
                        </span>
                      </span>
                    </span>
                    <span className={`transition-all duration-500 ${on ? "translate-x-0 text-[#8c6d46] opacity-100" : "-translate-x-2 opacity-0"}`}>→</span>
                  </button>
                </li>
              );
            })}
          ...
          <div className="order-1 lg:order-2 lg:col-span-6">
            <div className="lg:sticky lg:top-28">
              <div className="relative aspect-[4/3] overflow-hidden bg-[#e8e2d5]" data-cursor="Khám phá">
                {PLACES.map((p, i) => (
                  <img
                    key={p.id}
                    src={p.img}
                    ...
                    className="img-grade absolute inset-0 h-full w-full object-cover transition-all duration-[1100ms]"
              ...
              <div key={cur.id} className="kenburns mt-4 flex items-start justify-between gap-6">
                ...
                <a href={cur.href} className="link-line shrink-0 text-[11px] font-semibold uppercase tracking-[0.22em] text-[#8c6d46]">
                  Khám phá →
                </a>
              </div>
            </div>
